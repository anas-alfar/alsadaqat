...
class OrganizationPosition extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('organization_id, title, description, qualifications, responsibilities, owner_id, created_at, updated_at, notes, options', 'required'),
			array('organization_id, owner_id', 'length', 'max'=>11),
			array('title, description, qualifications, responsibilities', 'length', 'max'=>255),
			array('notes, options', 'length', 'max'=>1024),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, organization_id, title, description, qualifications, responsibilities, owner_id, created_at, updated_at, notes, options', 'safe', 'on'=>'search'),
		);
	}
