...
class OrganizationAccessLevel extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('organization_id, title, description, created_at, updated_at, notes, options', 'required'),
			array('organization_id', 'length', 'max'=>11),
			array('title, description', 'length', 'max'=>255),
			array('notes, options', 'length', 'max'=>1024),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, organization_id, title, description, created_at, updated_at, notes, options', 'safe', 'on'=>'search'),
		);
	}
