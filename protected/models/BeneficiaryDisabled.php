 ...
 */
class BeneficiaryDisabled extends Aulaula
{
    public $mother;
	/**
    ...
    public function behaviors() {
        return array(
            'preview' => array(
                ...
                'directory' => Yii::getPathOfAlias('webroot'). '/beneficiaries/poor/'. $this->id. '/' .substr(sha1($this->id), 0, 10).'/'.substr(sha1($this->id), -10) . '/' . $this->id % 100, //Controller::getImagePath($this->id, 'donators'),
                // url for images folder
                'url' => Yii::app()->request->baseUrl . '/beneficiaries/poor/'. $this->id . '/' . substr(sha1($this->id), 0, 10). '/' .substr(sha1($this->id), -10) . '/' . $this->id % 100, //Controller::getImagePath($this->id, 'donators'),
                // image versions
                'versions' => array(
                    'small' => array(
            ...
            'motherDeathCertificate' => array(
                ...
                'extension' => 'jpg',
                // folder to store images
                'directory' => Yii::getPathOfAlias('webroot'). '/beneficiaries/disabled/'. $this->id. '/' .substr(sha1($this->id), 0, 10).'/'.substr(sha1($this->id), -10) . '/mother/' . $this->id % 100,
                // url for images folder
                'url' => Yii::app()->request->baseUrl . '/beneficiaries/disabled/'. $this->id . '/' . substr(sha1($this->id), 0, 10). '/' .substr(sha1($this->id), -10) . '/mother/' . $this->id % 100,
                // image versions
                'versions' => array(
                    'small' => array(
                        'resize' => array(70, null),
