...
/**
 * This is the model class for table "beneficiary_orphan".
 *
 * The followings are the available columns in table 'beneficiary_orphan':
 * @property string $id
 ...
 */
class BeneficiaryOrphan extends Aulaula
{
    public $mother;
	/**
	...
	public function rules()
	{
		...
		return array(
			...
			array('father_full_name, mother_full_name, mother_job, number_of_brothers, number_of_sisters, beneficiary_id', 'required'),
			...
			array('is_father_alive, father_job, father_date_of_birth, father_date_of_death, mother_date_of_birth, mother_date_of_death, created_at, updated_at', 'safe'),
	...
	{
		return array(
			'id' => Yii::t('beneficiary_orphan','ID'),
			'father_full_name' => Yii::t('beneficiary_orphan','Father Full Name'),
			'is_father_alive' => Yii::t('beneficiary_orphan','Is Father Alive'),
			'father_job' => Yii::t('beneficiary_orphan','Father Job'),
			'father_date_of_birth' => Yii::t('beneficiary_orphan','Father Date Of Birth'),
			'father_date_of_death' => Yii::t('beneficiary_orphan','Father Date Of Death'),
			'father_reason_of_death' => Yii::t('beneficiary_orphan','Father Reason Of Death'),
			'father_death_certificate_path' => Yii::t('beneficiary_orphan','Father Death Certificate Path'),
			'mother_full_name' => Yii::t('beneficiary_orphan','Mother Full Name'),
			'is_mother_alive' => Yii::t('beneficiary_orphan','Is Mother Alive'),
			'mother_job' => Yii::t('beneficiary_orphan','Mother Job'),
			'mother_date_of_birth' => Yii::t('beneficiary_orphan','Mother Date Of Birth'),
			'mother_date_of_death' => Yii::t('beneficiary_orphan','Mother Date Of Death'),
			'mother_reason_of_death' => Yii::t('beneficiary_orphan','Mother Reason Of Death'),
			'mother_death_certificate_path' => Yii::t('beneficiary_orphan','Mother Death Certificate Path'),
			'number_of_brothers' => Yii::t('beneficiary_orphan','Number Of Brothers'),
			'number_of_sisters' => Yii::t('beneficiary_orphan','Number Of Sisters'),
			'beneficiary_id' => Yii::t('beneficiary_orphan','Beneficiary'),
			'owner_id' => Yii::t('beneficiary_orphan','Owner'),
			'created_at' => Yii::t('beneficiary_orphan','Created At'),
			'updated_at' => Yii::t('beneficiary_orphan','Updated At'),
		);
	}
	 ...
	 * @return BeneficiaryOrphan the static model class
	 */
    ...
    public function behaviors() {
        return array(
            'preview' => array(
                ...
                'directory' => Yii::getPathOfAlias('webroot'). '/beneficiaries/orphan/'. $this->id. '/' .substr(sha1($this->id), 0, 10).'/'.substr(sha1($this->id), -10) . '/' . $this->id % 100, //Controller::getImagePath($this->id, 'donators'),
                // url for images folder
                'url' => Yii::app()->request->baseUrl . '/beneficiaries/orphan/'. $this->id . '/' . substr(sha1($this->id), 0, 10). '/' .substr(sha1($this->id), -10) . '/' . $this->id % 100, //Controller::getImagePath($this->id, 'donators'),
                // image versions
                'versions' => array(
                    'small' => array(
            ...
            'motherDeathCertificate' => array(
                ...
                'extension' => 'jpg',
                // folder to store images
                'directory' => Yii::getPathOfAlias('webroot'). '/beneficiaries/orphan/'. $this->id. '/' .substr(sha1($this->id), 0, 10).'/'.substr(sha1($this->id), -10) . '/mother/' . $this->id % 100,
                // url for images folder
                'url' => Yii::app()->request->baseUrl . '/beneficiaries/orphan/'. $this->id . '/' . substr(sha1($this->id), 0, 10). '/' .substr(sha1($this->id), -10) . '/mother/' . $this->id % 100,
                // image versions
                'versions' => array(
                    'small' => array(
                        'resize' => array(70, null),
